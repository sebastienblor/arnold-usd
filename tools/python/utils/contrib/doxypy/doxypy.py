...
class Doxypy(object):
	...
	def makeCommentBlock(self):
		...
		# kik: transform epydoc fields into doxygen tags
		epydoc_field_noarg_re = re.compile('@([^ :]+):')
		epydoc_field_arg_re = re.compile('@([^ :]+) +([^ :]+):')
		
		epydoc2doxygen = { 'raise':'throw' }
		
		for i in xrange(len(commentLines)):
			is_matched = False
			has_arg = False
			
			mo = epydoc_field_noarg_re.search(commentLines[i])
			if mo:
				is_matched = True
			else:
				mo = epydoc_field_arg_re.search(commentLines[i])
				if mo:	
					is_matched = has_arg = True
			
			if is_matched:
				token = mo.group(1)
				if token in epydoc2doxygen:
					token = epydoc2doxygen[token]

				if has_arg:
					commentLines[i] = epydoc_field_arg_re.sub('@%s %s' % (token, mo.group(2)), commentLines[i])
				else:
					commentLines[i] = epydoc_field_noarg_re.sub('@' + token, commentLines[i])
		# /kik
