...
def aiExportFrame( self, frame, objFilename ):
    ...
    print "##### aiExportFrame ",objFilename
    cmds.arnoldExportAss(f=objFilename+".ass", s=True)
    '''cmds.Mayatomr( 	mi=True, exportFilter=7020488, 
                ...
                file=objFilename + ".mi" )'''
    print "##### aiExportFrame Exported"
    self.log( "assExport " + objFilename + ".ass")
...
def aiExport( self, objs, filename, lod, materialNS ):
    print "##### aiExport ",objs," ", filename, " ", lod, " ", materialNS, "\n"
...
def xgaiArchiveExport(selfid) :
    print "##### xgaiArchiveExport ",selfid
...
def xgaiArchiveExportInfo(selfid) :
    print "##### xgaiArchiveExportInfo ",selfid
    self = castSelf(selfid)
...
'''cmds.callbacks(addCallback=aiArchiveExport,
               hook="ArchiveExport",
               owner="arnold")'''
