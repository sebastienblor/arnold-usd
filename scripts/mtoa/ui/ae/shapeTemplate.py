...
def getTranslatorTemplate(nodeType, translatorName):
    """
    return a template class for the given nodeType, or None if one has not been registered
    """
    try:
...
class BaseTemplate(object):
    ...
    Once instantiated, the UI can be attached to an AE template via BaseTemplate.attachToAE(),
    or simply built as a normal UI via BaseTemplate.attachToUI()
    """
