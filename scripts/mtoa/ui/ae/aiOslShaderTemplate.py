...
class OSLSceneModel():
    ...
    def __compileOSLCode__(self):
        ...
        if compilation_errors == None or AiArrayGetNumElements(compilation_errors) == 0 :
...
class AEaiOslShaderTemplate(ShaderAETemplate):
    ...
    def codeStatusCreate(self, attrName):
        cmds.rowLayout(numberOfColumns=2, columnWidth2=(100,100), adjustableColumn = 2, columnAlign=(1, 'right'), columnAttach=[(1, 'both', 0), (2, 'both', 0)] )
    ...
    def codeStatusUpdate(self, attrName):
        cmds.text(self.compile_control, e = True, l = self.compileEnum[cmds.getAttr(attrName)])
    ...
    def compiler(self, attrName, path ):
        ...
        if oslMayaScene.compileState:
            cmds.setAttr(nodeName+'.compileStatus', 1)
            cmds.text(self.compile_control, e = True, l = self.compileEnum[1])
            om.MGlobal.displayInfo(" Code compiled successfully ")
        else:
            cmds.setAttr(nodeName+'.compileStatus', 2)
            cmds.text(self.compile_control, e = True, l = self.compileEnum[2])
    ...
    def setShaderCode(self, attrName):
        ...
        cmds.setAttr(nodeName+'.compileStatus', 0)
        cmds.text(self.compile_control, e = True, l = self.compileEnum[0])
