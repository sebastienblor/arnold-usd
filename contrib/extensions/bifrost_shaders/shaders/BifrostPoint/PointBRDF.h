...
void *VolumeBRDFSetupData ( const AtShaderGlobals *sg, const float firstG, const float secondG, const bool dual, const float blend )
{
	...
	data->rayDirection = AiV3Normalize( -sg->Rd );
	...
	if ( !AiV3isZero( sg->dPdu ) && AiV3Exists( sg->dPdu ) ) {
		...
		AiBuildLocalFramePolar( &(data->U), &(data->V), &(data->rayDirection) );
	}
...
static AtVector HGVolumeBRDFEvalSample (const void *brdf_data, float rx, float ry)
{
	...
	if ( data->dual ) {
		...
		AtVector vector1 = HGSample( &( data->firstG ), data->U, data->V, data->rayDirection, rx, ry );
		AtVector vector2 = HGSample( &( data->secondG ), data->U, data->V, data->rayDirection, rx, ry );

		return vector1 * data->blendInvert + vector2 * data->blend;
	} else {
		return HGSample( &( data->finalG ), data->U, data->V, data->rayDirection, rx, ry );
	}
}
...
static float HGVolumeBRDFEvalPDF( const void *brdf_data, AtVector *inDirection )
{
	...
	if ( data->dual ) {
		// eval both functions and interpolate
		float pdf1 = HGPDF( &( data->firstG ), data->rayDirection, inDirection );
		float pdf2 = HGPDF( &( data->secondG ), data->rayDirection, inDirection );

		return pdf1 * data->blendInvert + pdf2 * data->blend;
	} else {
		return HGPDF( &( data->finalG ), data->rayDirection, inDirection );
	}
}
...
static AtVector SchlickVolumeBRDFEvalSample (const void *brdf_data, float rx, float ry)
{
	...
	if ( data->dual ) {
		// eval both functions and interpolate
		AtVector vector1 = SchlickSample( &( data->firstG ), data->U, data->V, data->rayDirection, rx, ry );
		AtVector vector2 = SchlickSample( &( data->secondG ), data->U, data->V, data->rayDirection, rx, ry );

		return vector1 * data->blendInvert + vector2 * data->blend;
	} else {
		return SchlickSample( &( data->finalG ), data->U, data->V, data->rayDirection, rx, ry );
	}
}
...
static float SchlickVolumeBRDFEvalPDF( const void *brdf_data, AtVector *inDirection )
{
	...
	if ( data->dual ) {
		// eval both functions and interpolate
		float pdf1 = SchlickPDF( &( data->firstG ), data->rayDirection, inDirection );
		float pdf2 = SchlickPDF( &( data->secondG ), data->rayDirection, inDirection );

		return pdf1 * data->blendInvert + pdf2 * data->blend;
	} else {
		return SchlickPDF( &( data->finalG ), data->rayDirection, inDirection );
	}
}
