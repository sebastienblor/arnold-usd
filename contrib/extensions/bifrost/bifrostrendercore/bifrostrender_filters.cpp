...
void displaceChannel2( InfCubeParams infCube, Bifrost::API::StateServer srcSS, FrameData *frameData, DiagnosticsParams diagnostics )
{
	Bifrost::API::Layout layout = frameData->srcChannel.layout();
	Bifrost::API::Component component = frameData->srcChannel.component();

	bool tmpCreated = false;
	// how are we doing the blending, with a channel or with a falloff map
	if ( frameData->infCubeBlendChannel.valid() ) {
		IFNOTSILENTINFUNCTION { printf( "\tBlending with an infinite cube using %s channel...\n\n", frameData->infCubeBlendChannel.partialPathName().c_str() ); }
		...
		IFNOTSILENTINFUNCTION { printf( "\tBlending with an infinite cube...\n" ); }
	}

	// compute sim boundary in world space
	computeImplicitsBounds( frameData, infCube, diagnostics );

	// to do this properly we are creating another voxel object and channel called displacement
	// then for each input tile we record the displacement per tile per voxel
	// than during lookup of values for rendering, instead of the input point we do lookup at inputpoint+displacementValue
	// if we simply move stuff around in the bif structure we would need to copy the whole bif structure (well at least some part of it ) which would be bad for memory consumption

	// create a new object model
	Bifrost::API::ObjectModel dispOM;

	// create state server
	Bifrost::API::StateServer dispSS = dispOM.createStateServer();

	// create top object
	Bifrost::API::Object dispObject = dispSS.createObject("DispVoxel", "displacement");
	// create a component
	Bifrost::API::Component dispComponent = dispSS.createComponent( Bifrost::API::VoxelComponentType, "displacement-volume", layout.voxelScale() );
	// add it to the main object
	dispObject.addComponent( dispComponent );
	// create a channel in the component
	// create a float instead of a vector now as we only care about vertical displacement
	// by changing that to a vector we can also do displacement
	frameData->displacementChannel = dispSS.createChannel( dispComponent, Bifrost::API::FloatType, "displacement" );
	// set values for channel
	frameData->displacementChannel.setOffsetType ( frameData->srcChannel.offsetType() );
	frameData->displacementChannel.setBackgroundValue <float> ( 0.0f );

	// set layout values
	Bifrost::API::Layout dispLayout = dispComponent.layout();

	// create the root of the tile tree
	Bifrost::API::TileAccessor dispAcc = dispLayout.tileAccessor();
	Bifrost::API::TreeIndex dispRoot = dispAcc.addRoot();

	// get tiles and initialise stuff
	amino::Math::vec3f voxelOffsetValue = frameData->srcChannel.offsetValue();
	float tileTopCenterY = infCube.topCenterY / layout.voxelScale();
	float fullDisp = ( infCube.topCenterY - infCube.simWaterLevel ) / layout.voxelScale();
	...
	while (inTileIter) {
		...
		const Bifrost::API::TileCoord inTileCoord = inTile.coord();
		const Bifrost::API::TreeIndex inTileIndex = inTile.index();
		const Bifrost::API::TileInfo inTileInfo = inTile.info();

		// are there any elements in this tile
		Bifrost::API::TileData<float> inTileData = frameData->srcChannel.tileData<float>( inTileIndex );
		Bifrost::API::TileData<float> blendData = frameData->infCubeBlendChannel.tileData<float>( inTileIndex );

		// for each voxel calc a displacement and depending on that calc a tile where we put our data in
		// if that tile does not exists add it and add our data to the corresponding voxel.
		for ( size_t voxel = 0; voxel < inTileData.count(); voxel++ ) {
			// get voxel indices
			int voxelI = int ( voxel % 5);
			int voxelJ = ( int ( voxel / 5) ) % 5;
			int voxelK = int ( voxel / 25);

			// get voxel offset from tile corner
			amino::Math::vec3f voxelOffset (	voxelI * inTileInfo.dimInfo.voxelWidth + voxelOffsetValue[0],
												voxelJ * inTileInfo.dimInfo.voxelWidth + voxelOffsetValue[1],
												voxelK * inTileInfo.dimInfo.voxelWidth + voxelOffsetValue[2]); 
			// calc voxel tile pos
			amino::Math::vec3f voxelPosTile (	float( inTileCoord.i ) + voxelOffset[0],
												float( inTileCoord.j ) + voxelOffset[1],
												float( inTileCoord.k ) + voxelOffset[2] );

			float verticalOffset = 0.0f;

			// calc blend in x and z
			// this is done such that, equal distances in tile space yield the same value for x and z directions
			float finalBlend = 0.0f;
			if ( frameData->infCubeBlendChannel.valid() ) {
				// we will use data provided by the user
				finalBlend = blendData[ voxel ];

				// calc the offset
				verticalOffset = finalBlend * fullDisp;
				voxelPosTile[ 1 ] += verticalOffset;
			} else {
				// we will calc a rectangular falloff
				float blendX = fabs ( 2 * ( voxelPosTile[0] - frameData->bboxSim.tile.center[0] ) );
				float blendZ = fabs ( 2 * ( voxelPosTile[2] - frameData->bboxSim.tile.center[2] ) );
				// take correctionfactor into account
				if ( frameData->bboxSim.tile.dim[2] > frameData->bboxSim.tile.dim[0] ) {
					// z is greater than x
					blendX /= frameData->bboxSim.tile.dim[0];
					blendZ = std::max(0.0f, blendZ - frameData->bboxSim.tile.dim[2] + frameData->bboxSim.tile.dim[0]);
					blendZ /= frameData->bboxSim.tile.dim[0];
				} else {
					// x is greater than z
					blendX = std::max(0.0f, blendX - frameData->bboxSim.tile.dim[0] + frameData->bboxSim.tile.dim[2]);
					blendX /= frameData->bboxSim.tile.dim[2];
					blendZ /= frameData->bboxSim.tile.dim[2];
				}
				...
				// for the rest we calc a vertical offset
				if ( finalBlend >= infCube.blendStart ) {
					if ( finalBlend >= infCube.blendEnd ) {
						// we are at the infinite surface part
						// so we need to apply full displacement
						verticalOffset = fullDisp;
						voxelPosTile[ 1 ] += verticalOffset;
					} else {
						// we are in between
						// calc a smooth finalblend value
						finalBlend = calcBlendVal ( infCube.blendType, infCube.blendStart, infCube.blendEnd, finalBlend);
					...
					// we do not do any warping so no displacement
					// which means our final point is voxelPosTile
					// so nothing to do
				}
			} // end blend channel check

			// so here we have a final voxelTilePos
			// if there is no tile at that point add it
			Bifrost::API::TreeIndex dispTileIndex = dispAcc.addTile( int( voxelPosTile[ 0 ] ), int( voxelPosTile[ 1 ] ), int( voxelPosTile[ 2 ] ), inTileIndex.depth );
			Bifrost::API::Tile dispTile = dispAcc.tile( dispTileIndex );

			// init tile data
			Bifrost::API::TileData<float> dispTileData = frameData->displacementChannel.tileData<float>( dispTileIndex );

			// fill tile data
			int tileDataI = clip <int> ( (int) floor( voxelPosTile[0] - dispTile.coord().i ), 0, 4);
			int tileDataJ = clip <int> ( (int) floor( voxelPosTile[1] - dispTile.coord().j ), 0, 4);
			int tileDataK = clip <int> ( (int) floor( voxelPosTile[2] - dispTile.coord().k ), 0, 4);

			dispTileData(tileDataI, tileDataJ, tileDataK) = verticalOffset;
		} // end inTileData data loop
