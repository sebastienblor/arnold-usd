...
#define DL std::cerr << "line " << __LINE__ << std::endl
#define DUMP(v) std::cerr << "line " << __LINE__ << ": " << #v << " = " << (v) << std::endl
...
procedural_init_bounds
{
	...
	*user_ptr = (void *) data;
    ...
    data->world2Obj = AiNodeGetMatrix ( node, "matrix" );
    data->obj2World = AiM4Invert ( data->world2Obj );

	data->camNode = AiUniverseGetCamera();
	AiWorldToCameraMatrix( data->camNode, 0.0f, data->world2Cam );
    data->obj2Cam = AiM4Mult( data->obj2World, data->world2Cam );
