...
inline bool getNodeParameters(ImplicitsInputData *inData, const AtNode *node);

inline CoreObjectUserData *createCoreObjectUserData(Bifrost::API::String& json, Bifrost::API::String& filename, Bifrost::API::String& writeToFolder);

inline void PostProcessVoxels(BifrostPolyMeshUserData *nodeData);

inline AtNode* ProcSubdivide(BifrostPolyMeshUserData *nodeData);
...
procedural_init
{
    AtBBox bnds; AtBBox* bounds = &bnds;
    ...
    PostProcessVoxels(data); // erode, dilate, smooth
    ...
    AtNode* polymesh = nodeData->inputData->error? NULL : ProcSubdivide( nodeData );
    *user_ptr = polymesh;

    printEndOutput( "[BIFROST POLYMESH] END OUTPUT", nodeData->inputData->diagnostics );

    if (nodeData->frameData) {
        //if ( inData->hotData ) {
        //    Bifrost::API::File::deleteFolder( frameData->tmpFolder );
        //}
        delete nodeData->frameData;
    }

    if(nodeData->inputData){
        delete nodeData->inputData;
    }
    delete nodeData;
...
node_parameters
{
    ...
    AiParameterInt( "infCubeOutputType" , 0);
    ...
    AiParameterFlt("infCubeDimX", 0);
    AiParameterFlt("infCubeDimZ", 0);
    AiParameterInt( "blendType" , 0);
    AiParameterFlt("infCubeBlendStart", 0);
    AiParameterFlt("infCubeBlendEnd", 0);
    AiParameterFlt("blendingChannelRemapMin", 0);
    AiParameterFlt("blendingChannelRemapMax", 0);
...
bool getNodeParameters( ImplicitsInputData *inData, const AtNode *node )
{
    ...
    DUMP(inData->cullSides.start = AiNodeGetFlt(node, "cullSidesStart"));
    DUMP(inData->cullSides.end = AiNodeGetFlt(node, "cullSidesEnd"));
    ...
    DUMP(inData->smooth.on = AiNodeGetBool( node, "smoothOn" ));
    ...
    DUMP(inData->smooth.remapMin = AiNodeGetFlt(node, "smoothRemapMin"));
    DUMP(inData->smooth.remapMax = AiNodeGetFlt(node, "smoothRemapMax"));
    DUMP(inData->smooth.remapInvert = AiNodeGetBool(node, "smoothRemapInvert"));

    DUMP(inData->clip.on = AiNodeGetBool( node, "clipOn" ));
    DUMP(inData->clip.minX = AiNodeGetFlt(node, "clipMinX"));
    DUMP(inData->clip.maxX = AiNodeGetFlt(node, "clipMaxX"));
    DUMP(inData->clip.minY = AiNodeGetFlt(node, "clipMinY"));
    DUMP(inData->clip.maxY = AiNodeGetFlt(node, "clipMaxY"));
    DUMP(inData->clip.minZ = AiNodeGetFlt(node, "clipMinZ"));
    DUMP(inData->clip.maxZ = AiNodeGetFlt(node, "clipMaxZ"));
    ...
    DUMP(inData->infCube.topCenterX = AiNodeGetFlt(node, "infCubeTopCenterX"));
    DUMP(inData->infCube.topCenterY = AiNodeGetFlt(node, "infCubeTopCenterY"));
    DUMP(inData->infCube.topCenterZ = AiNodeGetFlt(node, "infCubeTopCenterZ"));
    ...
    DUMP(inData->infCube.blendStart = AiNodeGetFlt(node, "infCubeBlendStart"));
    DUMP(inData->infCube.blendEnd = AiNodeGetFlt(node, "infCubeBlendEnd"));
    DUMP(inData->infCube.remapMin = AiNodeGetFlt(node, "blendingChannelRemapMin"));
    DUMP(inData->infCube.remapMax = AiNodeGetFlt(node, "blendingChannelRemapMax"));
...
void PostProcessVoxels(BifrostPolyMeshUserData *nodeData) {
    ...
    //
    if (inData->dilateAmount != 0.0f || inData->erodeAmount != 0.0f || ( inData->smooth.on && inData->smooth.amount > 0 && inData->smooth.iterations > 0 && inData->smooth.weight > 0.0 )) {
        IFNOTSILENT {
...
AtNode* ProcSubdivide( BifrostPolyMeshUserData *nodeData )
{
    printf("\nCreating the mesh...\n");
