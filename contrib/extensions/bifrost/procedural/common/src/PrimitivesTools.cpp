						...
						)
{
	...
	if ( inData->renderType == PRIM_POINT  || inData->renderType == PRIM_SPHERE ) {
		...
		for ( int i = 0; i < totalExported; i++ ) {
			// set position data
			int index = i * 3;
			AtPoint tmpPoint = AiPoint ( posData[ index ], posData[ index + 1 ], posData[ index + 2 ] );
			AiArraySetPnt(pointArray, i, tmpPoint);
			...
			if ( frameData->motionBlur ) {
				...
				tmpPoint = AiPoint ( posData[ index ], posData[ index + 1 ], posData[ index + 2 ] );
				AiArraySetPnt(pointArray, i + totalExported, tmpPoint);
		...
		if ( inData->exportNormalAsPrimvar && inData->renderType == PRIM_POINT ) {
			...
			for ( int i = 0; i < totalExported; i++ ) {
				int index = i * 3;
				AtVector tmpVector = AiVector ( normalData[ index ], normalData[ index + 1 ], normalData[ index + 2 ] );
