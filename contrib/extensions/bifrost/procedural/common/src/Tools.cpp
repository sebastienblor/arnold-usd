...
void getASSData ( void *frameData, void *inputData, PluginType pluginType )
{
	...
	if ( pluginType != PLUGIN_PRIMITIVES && pluginType != PLUGIN_MESH ) {
		...
		// get screenWindow
        AtVector2 tmp = AiNodeGetVec2( camNode, "screen_window_min" );
		( (FrameData *)frameData )->screenWindow[ 0 ] = tmp.x;
		( (FrameData *)frameData )->screenWindow[ 1 ] = tmp.y;

        tmp = AiNodeGetVec2( camNode, "screen_window_max" );
...
Bifrost::API::VoxelSampler * getAndSetThreadDataAI( std::string idString, Bifrost::API::VoxelChannel channel, void *usrData )
{
    ...
    std::map<uint16_t, Bifrost::API::VoxelSampler *>::iterator it;
	...
	if ( ! nodeData->samplerPool.empty() && nodeData->samplerPool.count( nodeData->bifrostCtx->tid ) != 0 ) {
        ...
        nodeData->samplerPool.insert( std::pair<uint16_t, Bifrost::API::VoxelSampler *>( nodeData->bifrostCtx->tid, channelSampler ) );
	}
...
{
    AtVector evalPoint  = AtVector( noiseP[0], noiseP[1], noiseP[2] );
...
void transformPointsAI( CvPoint finalWp[2], float shutterStart, void *usrData )
{
    ...
    AtVector evalPoint  = AtVector( finalWp[ 0 ][ 0 ], finalWp[ 1 ][ 1 ], finalWp[ 0 ][ 2 ] );
    AtVector xformed = AiM4PointByMatrixMult( nodeData->obj2Cam, evalPoint );
	finalWp[ 0 ][ 0 ] = xformed.x;
	finalWp[ 0 ][ 1 ] = xformed.y;
	finalWp[ 0 ][ 2 ] = xformed.z;


    evalPoint  = AtVector( finalWp[ 1 ][ 0 ], finalWp[ 1 ][ 1 ], finalWp[ 1 ][ 2 ] );
    xformed = AiM4PointByMatrixMult( nodeData->obj2Cam, evalPoint );
