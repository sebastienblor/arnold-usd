...
{
   m_main_window = new QMainWindow(0, Qt::WindowStaysOnTopHint);
   m_main_window->resize(w, h);
   m_main_window->setWindowTitle("Arnold Render View");
   m_gl = new CRenderGLWidget(m_main_window, *this, w, h);
...
void CRenderView::init(int w, int h)
{
   m_gl->resize(w, h);
   ...
   AiNodeSetFlt(driver, "gamma", K_gamma);
   ...
   */
   ...
   display_window.maxx = m_gl->width();
   display_window.maxy = m_gl->height();
