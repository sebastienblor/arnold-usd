...
static void GetAttributesFromNode(const MFnDependencyNode& node, unordered_map<std::string, std::string>& attributeList, std::string* fullName = 0)
{
   ...
   {      
      ...
      unordered_map<std::string, std::string>::iterator it = attributeList.find(attrName);
...
static void GetAttributesFromDgPath(const MDagPath& dg, unordered_map<std::string, std::string>& attributeList)
{
...
MStatus CArnoldListAttributesCmd::doIt(const MArgList& args)
{
   ...
   unordered_map<std::string, std::string> attributeMap;
   ...
   unsigned int id = 0;
   for (unordered_map<std::string, std::string>::const_iterator it = attributeMap.begin(); it != attributeMap.end(); ++it)
