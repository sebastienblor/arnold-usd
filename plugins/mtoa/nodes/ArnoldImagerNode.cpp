...
CAbMayaNode CArnoldImagerNode::s_abstract;
std::vector<CStaticAttrHelper> CArnoldImagerNode::s_nodeHelpers;

void* CArnoldImagerNode::creator()
{
   return new CArnoldImagerNode();
}

void CArnoldImagerNode::postConstructor()
{
...
MStatus CArnoldImagerNode::initialize()
{
   MFnAttribute fnAttr;
   ...
   // to add them to the list of existing imagers in the arnold menu
   CExtensionsManager::AddImager(maya);

   const AtNodeEntry *nodeEntry = AiNodeEntryLookUp(arnold.asChar());

   CStaticAttrHelper helper(CArnoldImagerNode::addAttribute, nodeEntry);
