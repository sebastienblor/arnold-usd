...
AtNode* CArnoldVolumeTranslator::ExportInstance(AtNode *instance, const MDagPath& masterInstance)
{
   ...
   AtByte visibility = AiNodeGetByte(masterNode, str::visibility);
...
void CArnoldVolumeTranslator::ExportVolumeShaders(AtNode* volume)
{
   ...
   if (!shadingGroupPlug.isNull())
   {
      ...
      if (shader != NULL)
      {
         AiNodeSetPtr(volume, str::shader, shader);
      ...
      else
      {
         ...
         AiNodeSetPtr(volume, str::shader, NULL);
      }
   }
}
...
{
   AiNodeSetStr(volume, str::name, AtString(GetSessionOptions().GetArnoldNaming(m_dagPath).asChar()));
   ...
   if (!update)
   {
      ...
      {
         AiNodeSetVec(volume, str::min,
                        -1.f,
                        -1.f,
                        -1.f);

         AiNodeSetVec(volume, str::max,
                        1.f,
                        1.f,
                        1.f);
      ...
      else
      {
         ...
         AiNodeSetStr( volume, str::filename, AtString(newFilename.asChar()));
         ...
         if (gridList.length() > 0)
         {
            ...
            AiNodeSetArray( volume, AtString("grids"), ary);
         }

         AiNodeSetFlt(volume, str::step_scale, m_DagNode.findPlug("stepScale", true).asFloat());
         ...
         if (vGridList.length() > 0)
         {
            ...
            AiNodeSetArray( volume, str::velocity_grids, ary);
         }

         AiNodeSetFlt(volume, str::velocity_scale, m_DagNode.findPlug("velocityScale", true).asFloat());

         AiNodeSetFlt(volume, str::velocity_fps, m_DagNode.findPlug("velocityFps", true).asFloat());

         AiNodeSetFlt(volume, str::velocity_outlier_threshold, m_DagNode.findPlug("velocityThreshold", true).asFloat());

         AiNodeSetBool(volume, str::disable_ray_extents, m_DagNode.findPlug("disableRayExtents", true).asBool());
         AiNodeSetBool(volume, str::compress, m_DagNode.findPlug("compress", true).asBool());
         AiNodeSetFlt(volume, str::volume_padding, m_DagNode.findPlug("volumePadding", true).asFloat());

         if (strcmp(nodeType, "volume_implicit") == 0)
         {
            // export as implicit
            AiNodeSetFlt(volume, str::threshold, m_DagNode.findPlug("threshold", true).asFloat());
            AiNodeSetUInt(volume, str::samples, m_DagNode.findPlug("samples", true).asInt());
            AiNodeSetInt(volume, str::solver, m_DagNode.findPlug("solver", true).asInt());
            AiNodeSetStr(volume, str::field_channel, AtString(m_DagNode.findPlug("fieldChannel", true).asString().asChar()));

            MPlugArray conns;
            MPlug field = FindMayaPlug("field");
            field.connectedTo(conns, true, false);
            if (conns.length() > 0)
               AiNodeSetPtr(volume, str::field, ExportConnectedNode(conns[0]));
            else
               AiNodeSetPtr(volume, str::field, NULL);
