...
{
   AtNode *shader = AddArnoldNode("ramp_rgb");
   m_hasColorCorrect = RequiresColorCorrect();
...
{
   AtNode *colorCorrectNode = m_hasColorCorrect ? GetArnoldNode("cc") : NULL;
   AtNode *uvTransformNode = NULL;
   AtNode *rampFloat = NULL;

   // rampShader is the ramp node that will be used to set the type
   AtNode *rampShader = shader;

   AiNodeUnlink(shader, "input");
   if (RequiresUvTransform())
   {
      uvTransformNode = GetArnoldNode("uv");
      if (uvTransformNode == NULL)
         uvTransformNode = AddArnoldNode("uv_transform", "uv");

      rampFloat = GetArnoldNode("ramp_float");
      if (rampFloat == NULL)
         rampFloat = AddArnoldNode("ramp_float", "ramp_float");

      rampShader = rampFloat;
   }

   RampType type = (RampType)FindMayaPlug("type").asInt();
   switch(type)
   {
      default:
      case RT_V:
   ...
   if (FindMayaPlug("curveImplicitUvs").asBool())
   ...
   if (colorCorrectNode)
   {
      ...
      ProcessParameter(colorCorrectNode, "multiply", AI_TYPE_RGB, "colorGain");
      ProcessParameter(colorCorrectNode, "add", AI_TYPE_RGB, "colorOffset");
      AiNodeLink(shader, "input", colorCorrectNode);
   ...
   if (uvTransformNode)
   {
      ...
      if (hasNoise)
      {
         ...
         if (uWave || vWave)
         {
            ...
            AiNodeResetParameter(uvTransformNode, "passthrough");
            AiNodeLink(uvTransformWave, "passthrough", uvTransformNode);
            AiNodeLink(rampFloat, "passthrough", uvTransformWave);
         } 
      } 
      if (vWave || uWave)
      {
         ...
         AiNodeLink(mult, "offset", uvTransformWave);
