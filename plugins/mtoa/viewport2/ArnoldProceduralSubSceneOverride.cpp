...
namespace {
    void standInNodeDirtyPlugCallback(MObject& /*node*/, MPlug& plug, void* clientData)
    {
        ...
        static_cast<CArnoldProceduralSubSceneOverride*>(clientData)->invalidate(reuse);
    }
    ...
    void standInAttributeChangedCallback(MNodeMessage::AttributeMessage msg, MPlug & plug, MPlug & otherPlug,void* clientData)
    {
        ...
        if(attrName == "renderLayerInfo")
            static_cast<CArnoldProceduralSubSceneOverride*>(clientData)->invalidate(false);
    }
    ...
    void globalOptionsChanged(MNodeMessage::AttributeMessage, MPlug& plug, MPlug&, void *clientData)
    {
        // invalidate the draw data if the global draw override changes.
        MString plugName = plug.partialName(false, false, false, false, false, true);
        if (plugName == MString("standinDrawOverride"))
            static_cast<CArnoldProceduralSubSceneOverride*>(clientData)->invalidate(true);
    ...
    {
    public:
        StandInShadedItemUserData(CArnoldProceduralSubSceneOverride* ov)
            : MUserData(true), fOverride(ov) {}
        ~StandInShadedItemUserData() { fOverride = NULL; }

        CArnoldProceduralSubSceneOverride* fOverride;
    };
...
CArnoldProceduralSubSceneOverride::CArnoldProceduralSubSceneOverride(const MObject& obj)
...
CArnoldProceduralSubSceneOverride::~CArnoldProceduralSubSceneOverride()
{
	MHWRender::MRenderer* renderer = MHWRender::MRenderer::theRenderer();
	if (renderer)
	{
...
void CArnoldProceduralSubSceneOverride::globalOptionsAdded(MObject& node, void* clientData)
{
    CArnoldProceduralSubSceneOverride* ssOverride = static_cast<CArnoldProceduralSubSceneOverride*>(clientData);
    ssOverride->globalOptionsAdded(node);
}

void CArnoldProceduralSubSceneOverride::globalOptionsAdded(MObject& node)
{
    if (mGlobalOptionsChangedID != 0)
        MNodeMessage::removeCallback(mGlobalOptionsChangedID);
...
MHWRender::DrawAPI CArnoldProceduralSubSceneOverride::supportedDrawAPIs() const
{
	// this plugin supports both GL and DX
	return (MHWRender::kAllDevices);
}

bool CArnoldProceduralSubSceneOverride::requiresUpdate(
    const MHWRender::MSubSceneContainer& container,
    const MHWRender::MFrameContext& frameContext) const
{
    CArnoldProceduralSubSceneOverride* nonConstThis = const_cast<CArnoldProceduralSubSceneOverride*> (this);
    return nonConstThis->anyChanges(container);
    //return true;
}

bool CArnoldProceduralSubSceneOverride::anyChanges(const MHWRender::MSubSceneContainer& container)
{
    MStatus status;
    MFnDagNode node(mLocatorNode, &status);
    ...
    MHWRender::MSubSceneContainer& container, const MHWRender::MFrameContext& frameContext)
{
    ...
    MStatus status;
    MFnDagNode node(mLocatorNode, &status);
    CArnoldBaseProcedural* standIn = static_cast<CArnoldBaseProcedural*>(node.userNode());
    CArnoldProceduralData* geom = NULL;
    ...
    if (wantBoxes)
    {
	 ...
	 bool hasSelection = (geom && geom->m_isSelected) && (hasLeadItem);
...
MHWRender::MRenderItem* CArnoldProceduralSubSceneOverride::getItem(
...
void CArnoldProceduralSubSceneOverride::updateWireframeCubeItem(CArnoldBaseProcedural* standIn, 
    MHWRender::MRenderItem* item, const MHWRender::MShaderInstance* shader, bool scaled)
{
    ...
    {
        CArnoldProceduralData* geom = standIn->geometry();
        bbMin = geom->m_bbox.min(); bbMax = geom->m_bbox.max();
    }
...
void CArnoldProceduralSubSceneOverride::updateRenderItem(MHWRender::MSubSceneContainer& container, CArnoldProceduralData* geom, 
...
size_t CArnoldProceduralSubSceneOverride::getIndexing(
    const CArnoldDrawGeometry& standIn, unsigned int* indices, unsigned int pointOffset, 
...
int CArnoldProceduralSubSceneOverride::getDrawOverride()
{
    MStatus status;
    MFnDagNode node(mLocatorNode, &status);
...
void CArnoldProceduralSubSceneOverride::updateShaderFromNode()
{
    MStatus status;
    MFnDagNode node(mLocatorNode, &status);
...
void CArnoldProceduralSubSceneOverride::releaseShadedMaterial()
{
    MHWRender::MRenderer* renderer = MHWRender::MRenderer::theRenderer();
    if (renderer)
    {
...
bool CArnoldProceduralSubSceneOverride::updateInstanceData(const MDagPathArray& instances)
{
...
void CArnoldProceduralSubSceneOverride::getInstanceTransforms( 
    MMatrixArray& instanceMatrixArray, 
    MMatrixArray& selectedInstanceMatrixArray, 
    MMatrixArray& unselectedInstanceMatrixArray,
    int& leadIndex)
...
bool CArnoldProceduralSubSceneOverride::getInstancedSelectionPath(
