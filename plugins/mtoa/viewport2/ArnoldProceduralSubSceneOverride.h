...
#include "nodes/shape/ArnoldDrawGeometry.h"

class CArnoldBaseProcedural;
class CArnoldProceduralData;

class DLLEXPORT CArnoldProceduralSubSceneOverride : public MHWRender::MPxSubSceneOverride
{
public:
	static MHWRender::MPxSubSceneOverride* Creator(const MObject& obj)
	{
		return new CArnoldProceduralSubSceneOverride(obj);
	}

	virtual ~CArnoldProceduralSubSceneOverride();
...
protected:
	...
	CArnoldProceduralSubSceneOverride(const MObject& obj);
    ...
    void updateWireframeCubeItem(CArnoldBaseProcedural* standIn, MHWRender::MRenderItem* wireframecube, 
                                 const MHWRender::MShaderInstance* shader, bool scaled);
    void updateRenderItem(MHWRender::MSubSceneContainer& container, CArnoldProceduralData* geom, 
    ...
    void fillBuffers(const CArnoldDrawGeometry& standIn, unsigned int* indices, float* vertices, float* normals, 
                     ...
                     bool wantNormals, bool boxMode, const AtMatrix *matrix = NULL);
    ...
    size_t getVertexStreams(const CArnoldDrawGeometry& standIn, float* vertices, float* normals, const AtMatrix *matrix);
